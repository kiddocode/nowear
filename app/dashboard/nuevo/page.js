    ...
    <div style={{display:'grid',gridTemplateColumns:'220px 1fr',minHeight:'calc(100vh - 68px)'}}>
      ...
      <main className="nuevo-grid" style={{display:'grid',gridTemplateColumns:'1fr 1fr',minHeight:'calc(100vh - 68px)'}}>
        ...
        <div className="nuevo-img" style={{position:'relative',overflow:'hidden'}}>
          <img src="https://images.unsplash.com/photo-1465495976277-4387d4b0b4c6?w=900&q=80" alt="Evento"
        ...
        <div style={{position:'fixed',inset:0,background:'rgba(10,10,10,0.7)',zIndex:2000,display:'flex',alignItems:'center',justifyContent:'center',padding:'1.5rem'}}>
          <div style={{background:'#FFFFFF',maxWidth:'860px',width:'100%',maxHeight:'90vh',overflowY:'auto'}}>
            <div style={{padding:'2.5rem 3rem 2rem',borderBottom:'1px solid #E0E0DC',display:'flex',justifyContent:'space-between',alignItems:'flex-start'}}>
              ...
              <button onClick={() => setShowPlanes(false)}
            ...
            <div className="planes-grid" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'1px',background:'#E0E0DC',margin:'2rem 3rem'}}>
              {PLANES.map(plan => (
                <div key={plan.id} onClick={() => setPlanSeleccionado(plan.id)}
                  ...
                  {plan.popular && (
                    <div style={{position:'absolute',top:'1rem',right:'1rem',fontSize:'0.52rem',fontWeight:600,letterSpacing:'0.1em',textTransform:'uppercase',background:'#C4917C',color:'#FFFFFF',padding:'0.2rem 0.5rem'}}>Popular</div>
                  )}
                  <div style={{fontSize:'2rem',fontWeight:100,color:planSeleccionado===plan.id?'#FFFFFF':'#0A0A0A',letterSpacing:'-0.03em',lineHeight:1,marginBottom:'0.25rem'}}>{plan.precio}</div>
                  <div style={{fontSize:'0.62rem',fontWeight:600,letterSpacing:'0.12em',textTransform:'uppercase',color:planSeleccionado===plan.id?'#C4917C':'#888884',marginBottom:'0.25rem'}}>{plan.duracion}</div>
                  <div style={{fontSize:'0.85rem',fontWeight:300,color:planSeleccionado===plan.id?'#FFFFFF':'#0A0A0A',marginBottom:'1.5rem'}}>{plan.nombre}</div>
            ...
            <div className="planes-footer" style={{padding:'1.5rem 3rem 2.5rem',display:'flex',justifyContent:'space-between',alignItems:'center'}}>
              ...
              <button onClick={handlePagar} disabled={!planSeleccionado||loading}
