...
export default function Register() {
  ...
  async function handleRegister() {
    ...
    if (data.user) { await supabase.from('profiles').update({ nombre }).eq('id', data.user.id) }
    await fetch('/api/admin/nuevousuario', { method: 'POST', headers: { 'Content-Type': 'application/json' }, body: JSON.stringify({ email, nombre, proveedor: 'email', created_at: new Date().toISOString() }) })
