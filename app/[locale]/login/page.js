...
export default function Login() {
  ...
  async function handleLogin() {
    ...
    setLoading(false)
    if (error) { setError(t('errorCredenciales')) } else { router.push('/dashboard') }
  }
    ...
    <div className="auth-grid" style={{display:'grid',gridTemplateColumns:'1fr 1fr',minHeight:'calc(100vh - 68px)'}}>
      ...
      <div style={{display:'flex',flexDirection:'column',justifyContent:'center',padding:'5rem 4rem',background:'#FFFFFF'}}>
        ...
        <div style={{marginBottom:'1.25rem'}}>
          ...
          <input type="password" value={password} onChange={e => setPassword(e.target.value)} onKeyDown={e => e.key === 'Enter' && handleLogin()} style={{width:'100%',fontFamily:'Poppins,sans-serif',fontSize:'0.82rem',fontWeight:300,padding:'0.9rem 1rem',border:'1px solid #E0E0DC',background:'#FFFFFF',outline:'none',boxSizing:'border-box'}}/>
        </div>
        ...
        <p style={{marginTop:'1.5rem',fontSize:'0.75rem',fontWeight:300,color:'#888884'}}>
          {t('sinCuenta')} <a href="/register" style={{color:'#0A0A0A',fontWeight:500,textDecoration:'underline',textUnderlineOffset:'3px'}}>{t('crearCuenta')}</a>
        </p>
      </div>
    </div>
  )
