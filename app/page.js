    ...
    <>
      ...
      <section className="hero-grid" style={{display:'grid',gridTemplateColumns:'1fr 1fr',minHeight:'calc(100vh - 68px)'}}>
        <div className="hero-left" style={{display:'flex',flexDirection:'column',justifyContent:'center',padding:'6rem 5rem 6rem 3rem'}}>
          <span style={{display:'inline-flex',alignItems:'center',gap:'0.6rem',fontSize:'0.62rem',fontWeight:500,letterSpacing:'0.16em',textTransform:'uppercase',color:'#C4917C',marginBottom:'2.5rem'}}>
            <span style={{width:'24px',height:'1px',background:'#C4917C',display:'inline-block'}}></span>
            Bodas · Comuniones · Bautizos · Eventos
          </span>
          <h1 style={{fontSize:'clamp(3rem,5vw,5.5rem)',fontWeight:100,lineHeight:1.06,letterSpacing:'-0.03em',marginBottom:'2rem'}}>
            Que nadie llegue<br/>
            <strong style={{fontWeight:600}}>vestida igual.</strong>
          </h1>
          <p style={{fontSize:'0.88rem',fontWeight:300,lineHeight:2,color:'#888884',maxWidth:'400px',marginBottom:'3rem'}}>
        ...
        <div className="hero-img-col" style={{position:'relative',overflow:'hidden'}}>
          <img src="https://images.unsplash.com/photo-1519741497674-611481863552?w=900&q=80" alt="Boda" style={{width:'100%',height:'100%',objectFit:'cover',display:'block'}}/>
          <div style={{position:'absolute',inset:0,background:'linear-gradient(to top, rgba(10,10,10,0.7) 0%, rgba(10,10,10,0.1) 60%)',display:'flex',flexDirection:'column',justifyContent:'flex-end',padding:'3rem'}}>
            <p style={{fontSize:'1.15rem',fontWeight:200,color:'#FFFFFF',lineHeight:1.55}}>
              Cada invitada llega<br/>
              <em style={{fontStyle:'italic',color:'#C4917C'}}>con su look único.</em>
            </p>
          </div>
        </div>
      </section>
      ...
      <section id="como" className="section-pad" style={{padding:'7rem 3rem'}}>
        <span style={{display:'flex',alignItems:'center',gap:'0.6rem',fontSize:'0.6rem',fontWeight:600,letterSpacing:'0.18em',textTransform:'uppercase',color:'#C4917C',marginBottom:'1.75rem'}}>
          <span style={{width:'24px',height:'1px',background:'#C4917C'}}></span>El proceso
        </span>
        <h2 style={{fontSize:'clamp(2.2rem,4vw,4rem)',fontWeight:100,lineHeight:1.08,letterSpacing:'-0.025em',marginBottom:'1.25rem'}}>
          Tres pasos.<br/><strong style={{fontWeight:600}}>Sin complicaciones.</strong>
        </h2>
        <p style={{fontSize:'0.85rem',fontWeight:300,lineHeight:2,color:'#888884',maxWidth:'480px',marginBottom:'4rem'}}>
          ...
          <div className="steps-img-col" style={{position:'relative',overflow:'hidden',minHeight:'480px'}}>
            ...
            <div style={{position:'absolute',inset:0,background:'linear-gradient(135deg,rgba(10,10,10,0.5) 0%,rgba(10,10,10,0.15) 100%)',display:'flex',alignItems:'flex-end',padding:'2.5rem'}}>
              <p style={{fontSize:'1.4rem',fontWeight:200,color:'#FFFFFF',lineHeight:1.45}}>
                Sin apps.<br/>Sin cuentas.<br/><em style={{color:'#C4917C'}}>Solo el link.</em>
              </p>
            </div>
          </div>
          <div style={{borderLeft:'1px solid #E0E0DC'}}>
            {[
              ...
              <div key={i} style={{padding:'2.5rem',display:'flex',gap:'2rem',borderBottom:i<2?'1px solid #E0E0DC':'none'}}>
                ...
                <div>
                  <div style={{fontSize:'0.9rem',fontWeight:600,marginBottom:'0.5rem'}}>{s.title}</div>
                  <div style={{fontSize:'0.78rem',fontWeight:300,color:'#888884',lineHeight:1.85}}>{s.body}</div>
                </div>
              </div>
            ))}
          </div>
      ...
      {/* PRICING */}
      <section className="section-pad" style={{padding:'7rem 3rem'}}>
        <span style={{display:'flex',alignItems:'center',gap:'0.6rem',fontSize:'0.6rem',fontWeight:600,letterSpacing:'0.18em',textTransform:'uppercase',color:'#C4917C',marginBottom:'1.75rem'}}>
          <span style={{width:'24px',height:'1px',background:'#C4917C'}}></span>Planes
        </span>
        <h2 style={{fontSize:'clamp(2.2rem,4vw,4rem)',fontWeight:100,lineHeight:1.08,letterSpacing:'-0.025em',marginBottom:'1.25rem'}}>
          Un pago único.<br/><strong style={{fontWeight:600}}>Por evento.</strong>
        </h2>
        <p style={{fontSize:'0.85rem',fontWeight:300,lineHeight:2,color:'#888884',maxWidth:'480px',marginBottom:'4rem'}}>
        ...
        <div className="pricing-grid" style={{display:'grid',gridTemplateColumns:'repeat(3,1fr)',gap:'1px',background:'#E0E0DC',border:'1px solid #E0E0DC'}}>
          {[
            ...
            <div key={i} style={{background:p.featured?'#0A0A0A':'#FFFFFF',padding:'2.5rem 2rem',position:'relative'}}>
              {p.featured&&<span style={{fontSize:'0.52rem',fontWeight:600,letterSpacing:'0.15em',textTransform:'uppercase',background:'#C4917C',color:'#FFFFFF',padding:'0.22rem 0.65rem',display:'inline-block',marginBottom:'1.25rem'}}>Más popular</span>}
              ...
              <div style={{fontSize:'0.75rem',fontWeight:300,lineHeight:1.8,color:'#888884',margin:'1.5rem 0',paddingTop:'1.5rem',borderTop:`1px solid ${p.featured?'#3A3A38':'#E0E0DC'}`}}>{p.desc}</div>
              {p.feats.map((f,j)=>(
                <div key={j} style={{display:'flex',gap:'0.55rem',fontSize:'0.73rem',fontWeight:300,color:p.featured?'#888884':'#3A3A38',marginBottom:'0.55rem'}}>
                  <span style={{color:'#C4917C',flexShrink:0}}>✓</span>{f}
                </div>
              ))}
              <a href="/register" style={{display:'block',textAlign:'center',marginTop:'2rem',padding:'0.9rem',fontSize:'0.78rem',fontWeight:500,background:p.featured?'#FFFFFF':'transparent',color:'#0A0A0A',border:p.featured?'none':'1px solid #0A0A0A',textDecoration:'none'}}>Empezar</a>
            </div>
          ))}
        </div>
      </section>
      ...
      <section id="contacto" className="contact-grid" style={{display:'grid',gridTemplateColumns:'1fr 1fr'}}>
        <div style={{background:'#0A0A0A',padding:'5rem 4rem',display:'flex',flexDirection:'column',justifyContent:'space-between',minHeight:'400px'}}>
          <div>
            <h2 style={{fontSize:'3rem',fontWeight:100,color:'#FFFFFF',lineHeight:1.08,letterSpacing:'-0.025em',marginBottom:'1rem'}}>
              ¿Alguna<br/><strong style={{fontWeight:600}}>pregunta?</strong>
            </h2>
            <p style={{fontSize:'0.8rem',fontWeight:300,color:'#888884',lineHeight:1.9}}>
              Estamos aquí para ayudarte. Te respondemos en menos de 24 horas.
            </p>
          </div>
          <div style={{fontSize:'0.82rem',fontWeight:400,color:'#C4917C'}}>support@nowear.es</div>
        </div>
        <div style={{padding:'5rem 4rem',background:'#F7F7F5'}}>
          {['Tu nombre','Email'].map((label,i)=>(
            <div key={i} style={{marginBottom:'1.25rem'}}>
              ...
              <input type={i===1?'email':'text'} style={{width:'100%',fontFamily:'Poppins,sans-serif',fontSize:'0.82rem',fontWeight:300,padding:'0.9rem 1rem',border:'1px solid #E0E0DC',background:'#FFFFFF',outline:'none'}}/>
            </div>
          ))}
          <div style={{marginBottom:'1.25rem'}}>
            ...
            <textarea style={{width:'100%',fontFamily:'Poppins,sans-serif',fontSize:'0.82rem',fontWeight:300,padding:'0.9rem 1rem',border:'1px solid #E0E0DC',background:'#FFFFFF',outline:'none',minHeight:'100px',resize:'vertical'}}/>
          </div>
          <button style={{width:'100%',padding:'0.9rem',fontSize:'0.78rem',fontWeight:500,background:'#0A0A0A',color:'#FFFFFF',border:'none',cursor:'pointer',fontFamily:'Poppins,sans-serif'}}>Enviar mensaje</button>
        </div>
      </section>
      
      {/* MARCAS */}
      <section id="marcas" className="section-pad" style={{padding:'7rem 3rem',background:'#F7F7F5'}}>
        <span style={{display:'flex',alignItems:'center',gap:'0.6rem',fontSize:'0.6rem',fontWeight:600,letterSpacing:'0.18em',textTransform:'uppercase',color:'#C4917C',marginBottom:'1.75rem'}}>
          <span style={{width:'24px',height:'1px',background:'#C4917C'}}></span>Marcas populares
        </span>
        <h2 style={{fontSize:'clamp(2.2rem,4vw,4rem)',fontWeight:100,lineHeight:1.08,letterSpacing:'-0.025em',marginBottom:'1.25rem'}}>
          Las tiendas que<br/><strong style={{fontWeight:600}}>más se registran.</strong>
        </h2>
        <p style={{fontSize:'0.85rem',fontWeight:300,lineHeight:2,color:'#888884',maxWidth:'480px',marginBottom:'4rem'}}>
        ...
        <div style={{display:'grid',gridTemplateColumns:'repeat(auto-fill,minmax(150px,1fr))',gap:'1px',background:'#E0E0DC',border:'1px solid #E0E0DC'}}>
          {['Zara','Mango','& Other Stories','Sandro','Massimo Dutti','H&M','Stradivarius','Pull&Bear','ASOS','Uterqüe','ba&sh','The Kooples','Maje','Claudie Pierlot','Isabel Marant','Rixo','Reiss','COS','Arket','Jacquemus','Self-Portrait','Ghost','Ted Baker','+ Sugerir marca'].map((brand,i)=>(
            <div key={i} style={{background:'#FFFFFF',padding:'1.75rem 1rem',display:'flex',alignItems:'center',justifyContent:'center',fontSize:'0.8rem',fontWeight:300,color:'#3A3A38',minHeight:'90px',textAlign:'center',cursor:'default',letterSpacing:'0.02em'}}>
              {brand}
            </div>
          ))}
        </div>
      ...
      {/* FAQ */}
      <section id="faq" className="section-pad" style={{padding:'7rem 3rem'}}>
        <div style={{display:'grid',gridTemplateColumns:'1fr 1fr',gap:'5rem',alignItems:'start'}}>
          <div style={{position:'sticky',top:'100px'}}>
            <span style={{display:'flex',alignItems:'center',gap:'0.6rem',fontSize:'0.6rem',fontWeight:600,letterSpacing:'0.18em',textTransform:'uppercase',color:'#C4917C',marginBottom:'1.75rem'}}>
              <span style={{width:'24px',height:'1px',background:'#C4917C'}}></span>FAQ
            </span>
            <h2 style={{fontSize:'clamp(2.2rem,4vw,4rem)',fontWeight:100,lineHeight:1.08,letterSpacing:'-0.025em',marginBottom:'1rem'}}>
              Todo lo que<br/><strong style={{fontWeight:600}}>necesitas saber.</strong>
            </h2>
            <p style={{fontSize:'0.82rem',fontWeight:300,lineHeight:2,color:'#888884',marginTop:'1rem'}}>
              ¿Más preguntas?<br/>
              <span style={{color:'#C4917C'}}>support@nowear.es</span>
            </p>
          </div>
          <div>
            {[
              ...
              <details key={i} style={{borderBottom:'1px solid #E0E0DC'}}>
                <summary style={{display:'flex',justifyContent:'space-between',alignItems:'center',padding:'1.5rem 0',fontSize:'0.85rem',fontWeight:400,color:'#0A0A0A',cursor:'pointer',listStyle:'none',letterSpacing:'-0.005em',lineHeight:1.4}}>
                  {item.q}
                  <span style={{fontSize:'1.3rem',fontWeight:100,color:'#BEBEBA',flexShrink:0,marginLeft:'1rem'}}>+</span>
                </summary>
                <p style={{fontSize:'0.78rem',fontWeight:300,color:'#888884',lineHeight:2,paddingBottom:'1.5rem'}}>{item.a}</p>
              </details>
            ))}
          </div>
        </div>
      ...
      <footer style={{background:'#0A0A0A',padding:'5rem 3rem 2.5rem'}}>
        <div className="footer-grid" style={{display:'grid',gridTemplateColumns:'2fr 1fr 1fr 1fr',gap:'3rem',marginBottom:'4rem'}}>
          <div>
            <div style={{fontSize:'1rem',fontWeight:600,letterSpacing:'0.18em',textTransform:'uppercase',color:'#FFFFFF',marginBottom:'1.25rem'}}>NOWEAR</div>
          ...
          {[
            ...
            <div key={i}>
              ...
              {col.links.map((link,j)=>(
                <div key={j} style={{fontSize:'0.75rem',fontWeight:300,color:'#888884',marginBottom:'0.65rem',cursor:'pointer'}}>{link}</div>
              ))}
            </div>
          ))}
        </div>
